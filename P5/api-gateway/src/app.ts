...
export function crearApp(): Application {
  ...
  app.use('/api/tickets', createProxyMiddleware({ target: TICKETS_URL, changeOrigin: true, pathRewrite: { '^/api/tickets': '/tickets' } }));
  app.use('/graphql/tickets', createProxyMiddleware({ target: TICKETS_URL, changeOrigin: true, pathRewrite: { '^/graphql/tickets': '/graphql' } }));

  app.use('/api/comentarios', createProxyMiddleware({ target: COMENTARIOS_URL, changeOrigin: true, pathRewrite: { '^/api/comentarios': '/comentarios' } }));
  app.use('/api/notificaciones', createProxyMiddleware({ target: NOTIFICACIONES_URL, changeOrigin: true, pathRewrite: { '^/api/notificaciones': '/notificaciones' } }));

  app.use((req, res) => res.status(404).json({ error: `Ruta ${req.method} ${req.originalUrl} no existe en el Gateway.` }));
