...
NS="${NS:-sa-prod}"
POD=$(kubectl get pod -n "$NS" -l app.kubernetes.io/component=db -o name | head -1)
...
sql() { kubectl exec -n "$NS" "$POD" -- psql -U "${PGUSER:-appuser}" -d "${PGDB:-sa_platform_db}" -tAc "$1"; }
...
echo ""
echo "Filas en prueba_dr : $(sql 'SELECT count(*) FROM prueba_dr;' 2>/dev/null || echo 'ERROR: consulta fallida')"
echo "Ultima marca       : $(sql 'SELECT marca FROM prueba_dr ORDER BY id DESC LIMIT 1;' 2>/dev/null || echo '-')"
echo "Tickets            : $(sql 'SELECT count(*) FROM tickets;' 2>/dev/null || echo 'ERROR: consulta fallida')"
echo "Usuarios           : $(sql 'SELECT count(*) FROM usuarios;' 2>/dev/null || echo 'ERROR: consulta fallida')"
echo "Comentarios        : $(sql 'SELECT count(*) FROM comentarios;' 2>/dev/null || echo 'ERROR: consulta fallida')"
